...
class LearningBasedController(EarlyExitStrategy):
    ...
    def should_exit(
        ...
        with torch.no_grad():
            ...
            # 2. Positional and potentially advanced scalars
            current_step_tokens = num_tokens
        ...
        if step_idx > 10 and prob < 0.10:
            print(
                f"  >>> HALLUCINATION CUTOFF TRIGGERED "
                f"(Confidence {prob*100:.2f}% < 10.00%). Pruning trace to save tokens."
            )
            return True
