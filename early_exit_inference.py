...
from mlp.train_controller import EarlyExitMLP

class EarlyExitPipeline:
    def __init__(self, llm_wrapper, controller_path="models/early_exit_controller.pt", scaler_path="models/scaler.pt", threshold=0.8, strategy=None):
    ...
    def generate_with_early_exit(self, prompt, max_steps=60, step_tokens=20, min_steps_before_check=4):
        ...
        for step_idx in range(max_steps):
            ...
            # --- Early Exit Check ---
            # Defer to external strategy if instantiated (modular approach)
            if hasattr(self, 'strategy') and self.strategy is not None:
                if self.strategy.should_exit(prompt, current_generation, step_info):
                    early_exit_triggered = True
                    stopped_at_step = step_idx
                    break
            else:
                # Built-in legacy monolithic approach (retained for backward compatibility if instantiated directly)
                with torch.no_grad():
