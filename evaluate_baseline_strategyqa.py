...
def evaluate_on_strategyqa(
    ...
    for idx, item in enumerate(tqdm(dataset)):
        ...
        # Baseline generation unconditionally overrides early exit
        generated_text = pipeline.llm.generate(prompt, max_new_tokens=1200)

        actual_tokens_used = len(pipeline.llm.tokenizer.encode(generated_text))
        extracted_ans = pipeline.extract_final_answer(generated_text, prompt=prompt).strip().lower()
    ...
    report_lines = [
        "=" * 55,
        f"  BASELINE METRICS: STRATEGY_QA ({split})",
        "=" * 55,
